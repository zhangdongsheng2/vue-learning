  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <ul class="activity">
          <li class="activity-item" :class="classSheetType[item.type]" v-for="(item,index) in info.supports"
              :key="index">
            <span class="content-tag" :class="classSheetType[item.type]"><span
                class="mini-tag">{{ item.name }}</span></span> <span class="activity-content">{{ item.content }}</span>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    _initScroll() {
      this.$nextTick(() => {
        const menuScroll = new BScroll('.shop-info', {
          click: true
        })
        this.$refs.picUl.style.width = 120 * this.info.pics.length + "px"
